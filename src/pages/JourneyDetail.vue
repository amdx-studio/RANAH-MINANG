...
const journey = computed(() => journeys.find(j => j.slug === route.params.slug) ?? journeys[0])

function bookJourney() {
  bookingStore.startBooking({
    slug: journey.value.slug,
    title: journey.value.title,
    price: journey.value.price,
  })
  router.push('/booking')
}
</script>
        ...
        <aside class="lg:sticky lg:top-28 lg:h-fit">
          <div class="card-surface p-6">
            ...
            <p class="mt-4 text-sm leading-relaxed text-charcoal/60">
              Includes local guides, culinary tastings, entrance fees, and community storytelling.
            </p>
